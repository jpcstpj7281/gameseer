...
class TestMain extends Sprite {
    ...
    function onLogin( msg:String, args:Array<Dynamic> , obj:Dynamic){
        if( msg == "LoginSucceed") {
            trace("try to load");
            //sw.loadMyComments();
            var func= null, arg1= null, arg2=null;
#if cpp
            func = "updateNewStatusWithImg";
            arg1 = "test3";

#elseif flash
            func= flash.Lib.current.root.loaderInfo.parameters.func;
            arg1= flash.Lib.current.root.loaderInfo.parameters.arg1;
            arg2= flash.Lib.current.root.loaderInfo.parameters.arg2;
#end
            ...
            if ( func != null ){
                if( arg1 != null){
                    if( arg2 != null){
                        Reflect.callMethod( sw, Reflect.field( sw, func), [arg1, arg2] ) ;
                    }else{
                        Reflect.callMethod( sw, Reflect.field( sw, func), [arg1] ) ;
                    }
                }else{
                    Reflect.callMethod( sw, Reflect.field( sw, func), [] ) ;
                }
            }
        }
        else if ( msg == "onData"){
            var data:Array<String> = args[0].split('{');
            var str:StringBuf = new StringBuf();

            for( d in data){
                str.addSub( "\n", 0);
                str.addSub( '{', 0);
                var data1:Array<String> = d.split('}');
                if( data1.length > 1){
                    for ( d1 in data1){
                        str.addSub( d1, 0);
                        str.addSub('}', 0);
