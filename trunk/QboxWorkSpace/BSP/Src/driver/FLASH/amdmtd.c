...
#define Am29LV033C_FLASH        0x01a3
...
FLStatus flwAmdMTDIdentify(FLFlash vol)
{
  ...
  #ifdef DEBUG_PRINT
    DEBUG_PRINT("Debug: interleave %x discovered vol.type=%x.\n",
	        inlv, vol.type);
  #endif
  ...
  if (vol.type == Am29LV033C_FLASH)
    vol.chipSize = 0x400000l;
  else if (vol.type == Am29F016_FLASH ||
      vol.type == Fuj29F016_FLASH ||
      vol.type == Am29F016C_FLASH ||
      vol.type == Fuj29F016C_FLASH ||
      vol.type == Am29LV017_FLASH ||
