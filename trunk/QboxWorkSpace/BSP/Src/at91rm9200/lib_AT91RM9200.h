...
__inline void AT91F_SSC_Configure (
             AT91PS_SSC pSSC,          /* \arg pointer to a SSC controller*/
             unsigned int ulSystClock,  /* \arg System Clock Frequency*/
             unsigned int ulBaudRate,   /* \arg Expected Baud Rate Frequency*/
             unsigned int ulClockRx,    /* \arg Receiver Clock Parameters*/
             unsigned int ulModeRx,     /* \arg mode Register to be programmed*/
             unsigned int ulClockTx,    /* \arg Transmitter Clock Parameters*/
             unsigned int ulModeTx)     /* \arg mode Register to be programmed*/
{
    /** Disable interrupts*/
	...
	AT91F_SSC_SetBaudrate(pSSC, ulSystClock, ulBaudRate);

     /** Write the Receive Clock Mode Register*/
	pSSC->SSC_RCMR =  ulClockRx;

     /** Write the Transmit Clock Mode Register*/
	pSSC->SSC_TCMR =  ulClockTx;

     /** Write the Receive Frame Mode Register*/
	pSSC->SSC_RFMR =  ulModeRx;

     /** Write the Transmit Frame Mode Register*/
	pSSC->SSC_TFMR =  ulModeTx;
...
/**----------------------------------------------------------------------------*/
/** \fn    AT91F_SSC_GetStatus*/
/** \brief Return SSC  Status*/
/**----------------------------------------------------------------------------*/
__inline unsigned int AT91F_SSC_GetStatus( /* \return SSC Interrupt Mask Status*/
        AT91PS_SSC pSsc) /* \arg  pointer to a SSC controller*/
{
        return pSsc->SSC_SR;
}
...
__inline unsigned int AT91F_US_Baudrate (
	...
	const unsigned int ulBaudRate)  /* \arg UART baudrate*/
{
	unsigned int baud_value = ((main_clock*10)/(ulBaudRate * 16));
	if ((baud_value % 10) >= 5)
		baud_value = (baud_value / 10) + 1;
	else
		baud_value /= 10;
...
#define  AT91F_GetTickCount() tickGet()
